---
title: "Redefining Uncertainty: Bayesian Modeling for Oceanography"
author: "Erdem M. Karaköylü"
date: 2025-09-29
description: End-to-end Bayesian modeling for chlorophyll-a prediction from ocean color.
categories:
  - Bayesian
  - Ocean Color
  - PyMC
...
Traditional satellite ocean-color algorithms often provide single-point chlorophyll-a estimates without calibrated uncertainty. This project implements a full **Bayesian workflow**—priors, sampling, posterior predictive checks, and model comparison—to produce **uncertainty-aware** predictions for chlorophyll-a.
...
- *“The probability that the quantity of interest lies between* $y_{\text{low}}$ *and* $y_{\text{high}}$ *is* $X\%$.”*  
...
**Why this approach.** Building on the long-standing **band-ratio (OCx) tradition**, I use the **Maximum Band Ratio (MBR)** as a compact, interpretable summary of spectral shape. Defining roles (indigo/blue/cyan/blue-green vs green/red) keeps the setup **sensor-agnostic**, and modeling a single covariate, $log_{10}(\mathrm{MBR})$, makes the workflow transparent. My contribution is to **apply a full Bayesian workflow**—with priors, partial pooling by numerator-band group, and a heteroskedastic likelihood—to this familiar framework to improve **predictive calibration** and interpretability.


**Concept (sensor-agnostic).** Following the band-ratio (OCx) tradition, I use the **Maximum Band Ratio (MBR)** to encode spectral shape: take the strongest short-wavelength (“blue family”) signal and normalize by a longer-wavelength green/red reference. Naming bands by **roles** (indigo/blue/cyan/blue-green vs green/red) keeps the definition sensor-agnostic:
...
- The **numerator** takes the maximum across several short-wavelength bands (capturing the strongest “blue” signal).  
...
**This study’s bands (SeaWiFS mapping).** I instantiate those roles with SeaWiFS-like bands:
- **indigo** $\approx 411$–$412$ nm $\rightarrow\ \mathrm{Rrs}_{411}$  
- **blue** $\approx 443$ nm $\rightarrow\ \mathrm{Rrs}_{443}$  
- **cyan** $\approx 488$–$490$ nm $\rightarrow\ \mathrm{Rrs}_{489}$  
- **blue-green** $\approx 510$ nm $\rightarrow\ \mathrm{Rrs}_{510}$  
- **green** $\approx 547$–$560$ nm $\rightarrow\ \mathrm{Rrs}_{555}$  
- **red** $\approx 667$–$681$ nm $\rightarrow\ \mathrm{Rrs}_{670}$
...
## Model Ladder
I use an iterative Bayesian workflow. Each cycle fits, diagnoses, and compares models, using those diagnostics to guide the next design—aiming to improve in-sample and out-of-sample fit and to assess whether predictive uncertainty is calibrated (i.e., that a stated 94% prediction interval contains about 94% of new observations the model hasn’t seen). Starting with the OC-style model below  are some of the models that make up the model ladder.

- **Model 1** — Baseline polynomial (OC-style)  
- **Model 2** — Hierarchical partial pooling  
- **Model 5** — Heteroskedastic likelihood  

Each step adds structure → improves **calibration** (not merely accuracy). Model 3 and 4 are not included here for brevity. Refer to the Supplementary Materials section in the preprint of my [preprint](https://doi.org/10.31223/X54J1J) for more details.
...
::: {.g-col-12}
![](figures/model1_structure_dag.png){fig-alt="Model 1 DAG — OC-style polynomial baseline: log-chl ~ f(log-MBR) with a single global (homoskedastic) noise scale." width=80%}
:::

::: {.g-col-12}
![](figures/model2_structure_dag.png){fig-alt="Model 2 DAG — Hierarchical partial pooling: group-level intercepts/slopes by MBR numerator with hyperpriors; shared noise scale." width=80%}
:::

::: {.g-col-12}
![](figures/model5_structure_dag.png){fig-alt="Model 5 DAG — Heteroskedastic hierarchical: partial pooling plus predictor/group-dependent noise (σ varies with x and/or group)." width=80%}
:::

:::
:::

Directed acyclic graphs of the three models’ generative structure. Nodes are variables/parameters; arrows indicate conditional dependence; plates denote repetition across groups or observations; shaded nodes are observed. The hierarchical and heteroskedastic edges explain why calibration improves across the ladder.
<br>
<u>$\Rightarrow$ How to read it</u>: Follow arrows from parameter priors → predictive mean/scale  → that conditions observed data $log(chl)$ via the likelihood. In Model 2, group plates and top level priors (hyperpriors) induce partial pooling through groups defined by the band in the MBR numerator; in Model 5, extra edges into $\sigma$ encode heteroskedastic noise, producing better tail behavior and near-nominal 94% coverage.
<br>
<u><b>Top (Model 1 — Baseline)</b></u>: Polynomial OC-style regression of log chlorophyll on log MBR with a single global noise scale (**homoskedastic**).  
<u><b>Middle (Model 2 — Hierarchical)</b></u>:Partial pooling** over numerator groups (plates): group-level intercepts/slopes with hyperpriors borrow strength across groups; common noise scale.  
<u><b>Bottom (Model 3 — Heteroskedastic)</b></u>: Hierarchical structure **plus heteroskedasticity**: the likelihood’s σ varies with predictor and/or group, improving tail fit and calibration.
...
:::
:::
**Left (Model 1 — Baseline):** Three-panel diagnostic — **PPC** (top) shows mismatched spread; **PIT KDE** (bottom-left) departs from flat; **ECDF − uniform** (bottom-right) crosses outside the 94% envelope → over/under-confidence.  
**Center (Model 2 — Hierarchical):** Partial pooling improves calibration — PPC envelopes better match variance, in spite of a conspicuous localized departure; PIT KDE flatter than for model 1; ECDF − uniform closer to zero with smaller deviations.  
**Right (Model 3 — Heteroskedastic):** Best calibration — PPC accommodates heteroskedastic spread; PIT KDE ≈ flat; ECDF − uniform remains within the 94% envelope across x.
:::
...
:::
:::
**Left (Model 1 — Baseline):** Empirical coverage vs nominal **0.94** shows pockets of under/over-coverage across log-MBR and numerator groups → intervals too narrow/wide in places.  
**Center (Model 2 — Hierarchical):** Coverage moves closer to nominal **0.94** by capturing group-specific means/variance; fewer systematic departures.  
**Right (Model 3 — Heteroskedastic):** Near-nominal **0.94** across predictors and groups; remaining mismatches are localized and small.
:::
...
PSIS-LOO model comparison (ΔELPD). Points show each model’s difference in expected log predictive density relative to the top model (higher is better); vertical bars are ±1 SE from the PSIS estimate. Intervals that overlap zero indicate no meaningful difference from the leader at this uncertainty level. Pareto-
𝑘
...
- **Calibration improves stepwise** from baseline → hierarchical → hierarchical with heteroskedasticity.  
...
**For researchers.**  
Uncertainty - as defined here = quantifies *what we still don’t know* given the model and data. This can show whether predictions match reality (via calibration and coverage plots as see above). This can also highlight where new measurements would help most and so inform future data collection efforts. Finally it provides a pregress indicator. We can ask, "have new models and data reduced uncertainty, i.e. are we more knowledgeable about the data generation process?", followed by the hard questions; "Why?", "Why not?" 

Thank you for reading.
---

## Links
- **Code** → Repo linked [here](https://github.com/erdemkarakoylu/bayesian_chl_pub)
- **Manuscript** → Preprint linked [here](https://doi.org/10.31223/X54J1J)
